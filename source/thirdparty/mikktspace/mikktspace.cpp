/** \file mikktspace/mikktspace.cpp
 *  \ingroup mikktspace
 */
/**
 ...
 *
 * -----------------------------------------------------------------------------
 * ALTERED SOURCE VERSION NOTICE (required by clause 2 above):
 *   This is an altered version of Morten S. Mikkelsen's original mikktspace.c.
 *   Modifications by the idtx-forge contributors (2026):
 *     - Renamed the translation unit from mikktspace.c to mikktspace.cpp so it
 *       participates in the C++ SCons build without a separate C compilation
 *       step. The source is compiled as C++ but is otherwise a byte-faithful
 *       copy of the reference implementation: no algorithm, data structure or
 *       numerical behaviour was changed, so the generated tangent spaces are
 *       identical to upstream. The public C API in mikktspace.h is unchanged.
 * -----------------------------------------------------------------------------
 */
